...
// Check current control
        ...
        if(control[0]) {
          ...
          var x = keypad.left + i * keypad.unit + keypad.gutter,
              y = (keypad.bottom + 40) - keypad.unit,
              key = Q.inputs[control[0]];

          // ctx.fillStyle = keypad.color || "#000000";
          ctx.globalAlpha = key ? 1.0 : 0.5;
          // ctx.fillRect(x,y,keypad.size,keypad.size);

// Draw images on buttons
          var img = new Image();

          switch(i) {
            case 0:
              img.src = "images/buttonleft.png";
              ctx.drawImage(img, x, y, keypad.size, keypad.size);
              break;
            case 1:
              img.src = "images/buttonright.png";
              ctx.drawImage(img, x, y, keypad.size, keypad.size);
              break;
            case 4:
  ...
  Q.component("platformerControls", {
    ...
    step: function(dt) {
      ...
      if(p.ignoreControls === undefined || !p.ignoreControls) {
        ...
        var originalSpeed = p.speed;
        var testSpeed = 0;

        if(Q.inputs['left']) {
          testSpeed = 0;
          temp_speed = temp_speed + 10;
          if(temp_speed > p.speed) {
            temp_speed = p.speed;
          }
          temp_direction = "left";
          testSpeed = testSpeed + temp_speed;
       ...
       //  document.getElementById("dt").innerHTML = testSpeed;
        
        if(Q.inputs['left']) {
          p.direction = 'left';
          if(collision && p.landed > 0) {
            p.vx = testSpeed * collision.normalY;
            p.vy = -testSpeed * collision.normalX;
          } else {
            p.vx = -testSpeed;
          }
        } else if(Q.inputs['right']) {
